...
def get_products(
    ...
    """
    try:
        logger.info(f"Getting products for query: {question}")
        ...
        logger.info(f"Successfully retrieved {len(product_dict)} products")
        return product_dict
    except Exception as e:
        logger.error(f"Product recommendation failed: {e}", exc_info=True)
        return []
...
class AgentFrameworkProductManagementAgent:
    ...
    def __init__(self):
        ...
        self.agent = ChatAgent(
            ...
            instructions=(
                ...
                '{"status": "input_required|completed|error", "message": "your response here"}\n'
                'Never respond with plain text - always use this JSON structure.'
            ),
    ...
    def _get_agent_response(
        ...
        try:
            ...
            # If JSON parsing fails, treat the message as plain text response
            logger.warning(f"Failed to parse structured response, using plain text: {e}")
            return {
                'is_task_complete': False,
                'require_user_input': True,
                'content': str(message),
